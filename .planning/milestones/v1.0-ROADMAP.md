...
- **1.1 Database Architecture**
  - [x] 配置 PostgreSQL + TimescaleDB + Redis。
  - [x] 定义 Drizzle Schema：增加 `categories` 表和 `results` 中的 `country_code` 字段。
- **1.2 Backend Skeleton**
  - [x] 初始化 Node.js TypeScript 环境，搭建租户与国家上下文。
  - [x] BullMQ 队列初始化。
...
- **2.1 Geo-Specific Adapters**
  - [x] 实现支持 `country_code` 参数的适配器（通过 Proxy 或 API 原生 Location 参数）。
  - [x] 集成 OpenAI, Perplexity, Gemini, Google AI Overviews。
- **2.2 Structured Parsing Service**
  - [x] 优化 LLM 解析逻辑，确保存储结果包含国家维度。
- **2.3 Prompt Explorer (POC)**
  - [x] 支持在 UI 选择国家并即时测试提问。

## Phase 3: Automation & Scheduled Jobs (Week 4)
*Goal: 实现全球化定时监测任务。*

- **3.1 Job Scheduler (Global)**
  - [x] 实现按国家和分类并行的监测任务派发逻辑。
...
- **5.1 Multi-Dim Aggregation**
  - [x] 优化聚合查询，支持平台/国家/分类的交叉分析。
- **5.2 Visualization Layer**
  - [x] 实现 Dashboard 筛选器与多维趋势图表。
- **5.3 Data Export**
...
## Phase 7: Gap Closure (MVP Completion)
- **7.1 Brand Management UI**: [GAP-01] 实现品牌/竞品管理 CRUD。
- **7.2 Dashboard Entry Integration**: [GAP-02] 根路径重定向与侧边栏导航。
